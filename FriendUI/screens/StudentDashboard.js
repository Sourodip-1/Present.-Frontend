...
const COLORS = {
  bgStart: '#FDFBF7', // Creamy off-white
  bgEnd: '#E6F4F1',   // Subtle light mint
  primaryStart: '#10B981', // Vibrant Green
  primaryEnd: '#059669',   // Deep Mint
  accentBlue: '#3B82F6',   // Neon soft blue
  accentRed: '#F43F5E',    // Warm red
  textDark: '#0F172A',
  textGray: '#64748B',
  textLight: '#94A3B8',
  white: '#FFFFFF',
...
const ATTENDED_DAYS = [2, 3, 5, 6, 9, 10, 11, 13, 16, 17, 19, 20, 23, 24];
const MISSED_DAYS = [4, 12, 18];

const NEARBY_BEACONS = [
  { id: '1', name: "Lab 3 Beacon (CS101)", rssi: -45 },
  { id: '2', name: 'Auditorium A', rssi: -65 },
  { id: '3', name: 'Main Library Entrance', rssi: -70 },
];

// ─── Shared Components ──────────────────────────────────────────────────────

// Faint background dotted pattern
const DotGrid = () => {
  const dotSpacing = 35;
      ...
      dots.push(
        <View
          ...
          style={{
            ...
            backgroundColor: COLORS.textLight, opacity: 0.15,
            left: j * dotSpacing + (i % 2 === 0 ? 0 : dotSpacing / 2), top: i * dotSpacing
          }}
        />
      );
    }
...
const GlassCard = ({ children, style, onPress, onLongPress }) => {
  ...
  const CardContent = (
    <Animated.View style={[{ transform: [{ scale }] }, styles.bentoCard, style]}>
      {children}
    </Animated.View>
  );
...
export default function StudentDashboard() {
  ...
  for (let i = 0; i < firstDay; i++) {
    calCells.push(null); // Blanks for days before the 1st
  }
  ...
  const sheetY = useRef(new Animated.Value(height)).current; // Start offscreen
  const lastSheetY = useRef(height);

  const snapPoints = {
    closed: height,
    half: height * 0.3, // Shows 70% of screen
    full: 0,           // Shows 100% of screen
  };
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gesture) => {
        ...
        if (newY < snapPoints.full) newY = snapPoints.full; // Clamp top
        if (newY > snapPoints.closed) newY = snapPoints.closed; // Clamp bottom
        sheetY.setValue(newY);
      },
      onPanResponderRelease: (_, gesture) => {
        let target = snapPoints.half;
        if (gesture.dy < -100 || (lastSheetY.current === snapPoints.half && gesture.dy < -20)) {
          target = snapPoints.full;
        } else if (gesture.dy > 100 || (lastSheetY.current === snapPoints.full && gesture.dy > 20)) {
  ...
  useEffect(() => {
    if (showScanner) {
      ...
      Animated.timing(sheetY, {
        toValue: snapPoints.closed,
        duration: 300,
        useNativeDriver: true,
      }).start();

      // Ensure scanning stops if sheet is dismissed mid-scan
      if (isScanning) {
  ...
  useEffect(() => {
    ...
    const p1 = Animated.loop(
      Animated.parallel([
        Animated.timing(pulseAnim1, { toValue: 1.8, duration: 2000, useNativeDriver: true }),
        Animated.timing(pulseOpac1, { toValue: 0, duration: 2000, useNativeDriver: true })
      ])
    );
    p1.start();
    ...
    setTimeout(() => {
      p2 = Animated.loop(
        Animated.parallel([
          Animated.timing(pulseAnim2, { toValue: 1.8, duration: 2000, useNativeDriver: true }),
          Animated.timing(pulseOpac2, { toValue: 0, duration: 2000, useNativeDriver: true })
        ])
      );
      p2.start();
    }, 1000);

    return () => { p1.stop(); if (p2) p2.stop(); };
  }, []);
  ...
  const handleScanInit = async () => {
    ...
    setTimeout(() => setNearbyDevices([NEARBY_BEACONS[0]]), 800);
    setTimeout(() => setNearbyDevices(prev => [...prev, NEARBY_BEACONS[1]]), 2000);
    setTimeout(() => setNearbyDevices(prev => [...prev, NEARBY_BEACONS[2]]), 3000);
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: COLORS.bgStart }]}>
      ...
      <View style={{ flex: 1, overflow: 'hidden', paddingTop: insets.top }}>
        ...
        <Animated.View style={[styles.header, { opacity: headerFade }]}>
          <View>
            ...
            <Text style={styles.studentUUID}>{STUDENT.uuid} • Year {STUDENT.year} • Sem {STUDENT.sem}</Text>
          </View>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.ctaContainer}>
              ...
              >
                <View style={[styles.scanBtn, attendanceMarked && styles.scanBtnDone]}>
                  <MaterialCommunityIcons
                    name={attendanceMarked ? 'check-decagram' : 'bluetooth-connect'}
                    size={50}
                    color={COLORS.white}
                  />
            ...
            <Text style={styles.ctaSub}>Tap to mark attendance</Text>
          </View>
          ...
          <View style={styles.statsLayout}>
            ...
            <View style={styles.statsLeft}>
              <GlassCard style={styles.mainStatCard} onPress={() => { }}>
                ...
                <View>
                  <Text style={styles.mainStatValue}>75%</Text>
                  <Text style={styles.statLabel}>Attendance</Text>
                </View>
                <View style={styles.trendBadge}>
                  <MaterialCommunityIcons name="trending-up" size={14} color={COLORS.primaryStart} />
                  <Text style={styles.trendText}>+5% this week</Text>
                </View>
              </GlassCard>
            </View>

            {/* Small Stats (Right Col) */}
            <View style={styles.statsRight}>
              <GlassCard style={styles.miniStatCard} onPress={() => { }}>
                ...
                <View style={styles.miniStatContent}>
                  <Text style={styles.miniStatValue}>8 Days</Text>
              ...
              <GlassCard style={styles.miniStatCard} onPress={() => { }}>
                ...
                <View style={styles.miniStatContent}>
                  <Text style={styles.miniStatValue}>142</Text>
          ...
          <GlassCard style={styles.calendarCard}>
            ...
            <View style={styles.calGrid}>
              {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((wd, i) => (
                <Text key={i} style={styles.calWdLabel}>{wd}</Text>
              ))}
              {calCells.map((d, i) => {
                const isAttended = d && ATTENDED_DAYS.includes(d) && currentMonth === 2; // Only for March in mock
                const isMissed = d && MISSED_DAYS.includes(d) && currentMonth === 2;
      ...
      {/* ─ Scanner Custom Draggable Sheet ─ */}
      <Animated.View 
      ...
      >
        {/* Backdrop for close */}
        <Animated.View 
          style={[styles.sheetBackdrop, { opacity: sheetY.interpolate({
            inputRange: [snapPoints.full, snapPoints.half, snapPoints.closed],
            outputRange: [1, 1, 0],
            extrapolate: 'clamp'
          })}]}
        >
          ...
          {!attendanceMarked ? (
            <>
              ...
              <View style={styles.radarContainer}>
                ...
                <Animated.View style={[styles.radarCircle, { 
                  transform: [{ scale: radarScale.interpolate({ inputRange: [1, 2.5], outputRange: [1, 2]}) }], 
                  opacity: radarOpacity.interpolate({ inputRange: [0, 1], outputRange: [0, 0.5]}) 
                }]} />
                <View style={styles.radarCore}>
                  <MaterialCommunityIcons name="antenna" size={32} color={COLORS.primaryEnd} />
                </View>
              </View>

              {nearbyDevices.length === 0 ? (
                <View style={styles.emptyDeviceList}>
                  <Text style={styles.scanningText}>Listening for signals...</Text>
                </View>
              ) : (
                  ...
                  {nearbyDevices.map((device, i) => (
                    <TouchableOpacity key={i} style={styles.deviceItem} onPress={() => handleDeviceSelect(device)}>
                      <View style={[styles.deviceIcon, { backgroundColor: '#D1FAE5' }]}>
                        <MaterialCommunityIcons name="bluetooth" size={20} color={COLORS.primaryEnd} />
                      </View>
                      <View style={styles.deviceInfo}>
                        <Text style={styles.deviceName}>{device.name}</Text>
                        <Text style={styles.deviceDist}>Tap to confirm attendance</Text>
                      </View>
            ...
            <View style={styles.successState}>
              <View style={styles.successIconWrap}>
                <MaterialCommunityIcons name="check-all" size={60} color={COLORS.white} />
              </View>
              <Text style={styles.successTitle}>Attendance Verified!</Text>
              <Text style={styles.successSub}>You are officially present for this session.</Text>
            </View>
          )}
        </View>
      </Animated.View>

      {/* ─ Calendar Day Info Modal (Now as Bottom Sheet) ─ */}
      <Modal visible={!!selectedDate} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          ...
          <View style={[styles.infoModal, { paddingBottom: insets.bottom + 20 }]}>
            <View style={styles.dragHandle} />
            <Text style={styles.infoModalTitle}>March {selectedDate}, 2026</Text>
            <GlassCard style={styles.infoCard}>
              <View style={styles.infoRow}>
                <MaterialCommunityIcons name="clock-outline" size={20} color={COLORS.textGray} />
                <Text style={styles.infoText}>09:00 AM - 11:00 AM</Text>
              </View>
              <View style={styles.infoRow}>
                <MaterialCommunityIcons name="book-open-outline" size={20} color={COLORS.textGray} />
                <Text style={styles.infoText}>Advanced Algorithms (CS304)</Text>
              </View>
              <View style={styles.infoBadge}>
                <Text style={styles.infoBadgeText}>
                  {ATTENDED_DAYS.includes(selectedDate) ? 'PRESENT' : MISSED_DAYS.includes(selectedDate) ? 'ABSENT' : 'UPCOMING'}
                </Text>
              </View>
            </GlassCard>
...
const styles = StyleSheet.create({
  ...
  scanBtn: {
    width: 140, height: 140, borderRadius: 70, backgroundColor: COLORS.primaryStart,
    alignItems: 'center', justifyContent: 'center',
    shadowColor: COLORS.primaryStart, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.4, shadowRadius: 20, elevation: 12,
  },
  scanBtnDone: { opacity: 0.9 },
  scanBtnText: { color: COLORS.white, fontSize: 15, fontWeight: '800', marginTop: 4, letterSpacing: 1 },
  glowRing: {
    position: 'absolute', width: 140, height: 140, borderRadius: 70,
    backgroundColor: COLORS.primaryStart, opacity: 0.5,
  },
  ctaSub: { fontSize: 14, color: COLORS.textGray, fontWeight: '600' },

  // Stats Layout (GenZ Style: 1 Big, 2 Small)
  statsLayout: { flexDirection: 'row', gap: 15, marginBottom: 25 },
  statsLeft: { flex: 1 },
  statsRight: { flex: 1, gap: 15, justifyContent: 'space-between' },

  mainStatCard: { flex: 1, minHeight: 220, justifyContent: 'space-between' },
  ...
  miniStatCard: { flex: 1, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 22, paddingVertical: 24 },
  ...
  sheetBackdrop: {
    position: 'absolute',
    top: -height, // Make sure it covers everything above the sheet
    left: 0,
    right: 0,
    height: height * 2,
  ...
  scannerContent: {
    ...
    left: 0,
    right: 0,
    height: height, // Full height available, we control view via translateY
    backgroundColor: '#FFFFFF',
  ...
  deviceIconWrapper: { width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
