...
export default function OTPScreen({ route, navigation }) {
  ...
  const [otp, setOtp] = useState(['', '', '', '']); // 4-digits for backend match
    ...
    try {
      ...
      if (response.ok) {
        if (data.isNewUser) {
          ...
          navigation.navigate(data.role === 'student' ? 'StudentDashboard' : 'TeacherDashboard', { phone });
        }
      } else {
      ...
      <View style={styles.flex}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
          <View style={[styles.topSection, { paddingTop: insets.top }]}>
            ...
            <DecryptedText text={`Sent to ${phone}`} style={styles.brandSubtitle} delay={500} />
          </View>
...
const styles = StyleSheet.create({
  ...
  otpInput: { width: 60, height: 65, backgroundColor: '#FFFFFF', borderRadius: 15, textAlign: 'center', fontSize: 26, fontWeight: '700', color: '#000' },
